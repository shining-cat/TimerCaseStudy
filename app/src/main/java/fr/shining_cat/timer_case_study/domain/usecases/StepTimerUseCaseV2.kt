    ...
    override suspend fun start(totalSeconds: Int) {
        ...
        return withContext(timerDispatcher) {
            ...
            initTimer(totalSeconds)
                .onCompletion {
                    hiitLogger.d("StepTimerUseCaseV2", "onCompletion:: actual step length = ${System.currentTimeMillis() - stepStartTimeStamp} | expected was: ${totalSeconds * 1000}")
                }
                .collect {
                    hiitLogger.d("StepTimerUseCaseV2","COLLECT: time since step start: ${System.currentTimeMillis() - stepStartTimeStamp}")
    ...
    private fun initTimer(totalSeconds: Int): Flow<StepTimerState> =
        ((totalSeconds - 1).times(timerHeartBeatRatio) downTo 0).asFlow() // first emit total - 1 because the total is emitted by onStart
            ...
            .transform { remainingHeartBeats: Int ->
                val remainingSeconds = remainingHeartBeats.div(timerHeartBeatRatio.toFloat()).toInt() // this rounding will hopefully correct for the drift?
                val now = System.currentTimeMillis()
                hiitLogger.d("StepTimerUseCaseV2","TICKING: remainingHeartBeats = $remainingHeartBeats | remainingSeconds = $remainingSeconds | tick length = ${now - loggingTimestamp} | time since step start: ${System.currentTimeMillis() - stepStartTimeStamp}")
